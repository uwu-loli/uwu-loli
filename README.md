...
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=uwu-loli&stats_format=bytes&layout=pie&hide=html&langs_count=18&theme=dark&bg_color=0d1117&hide_border=true&title_color=58a6ff&text_color=c9d1d9&icon_color=58a6ff)
...
```mermaid
graph LR
    A[Клиент] -->|Подписанная Загрузка| B[CDN Сервис]
    B -->|Валидация Токена| C[MongoDB]
    B -->|Сохранение Файла| D[Файловое Хранилище]
    C -->|Автоочистка| E[Фоновый Worker]
```
...
```mermaid
graph TB
    A[Tauri Frontend] -->|IPC| B[Rust Backend]
    B -->|SFTP| C[Удаленный Сервер]
    B -->|Шифрование| D[KeepassDB]
    D -->|Очистка RAM| E[Слой Безопасности]
```
